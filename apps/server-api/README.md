...
`apps/server-api` 是当前阶段基于 `Gin` 的 `Go` 服务端，负责管理员/老师/学生认证、教师账号管理、账号治理审计日志、任务上传与分配、学生进度、提示生成和教师实时看板接口。机器可读 API 契约以生成的 OpenAPI 为准。
...
- `PATCH /api/teacher/classes/{id}/students/{studentId}`
- `POST /api/teacher/classes/{id}/students/{studentId}/reset-password`
...
- `GET /api/teacher/projects/{id}/analysis`
- `POST /api/teacher/projects/{id}/assign-students`
- `POST /api/teacher/projects/{id}/publish`
- `POST /api/teacher/projects/{id}/archive`
- `GET /api/teacher/projects/{id}/live`
...
- 管理员对教师/学生执行创建、重置密码、启停、角色切换时，会自动写入 `audit_logs`，供 `/api/admin/audit-logs` 只读查询
