...
- 教师注册、登录、退出、`me`
- 教师创建班级、班级内单个创建学生、批量创建学生、重置学生密码
- 学生客户端登录、退出、`me`
- 教师在班级内上传参考 `sb3`
...
2. 教师先创建班级，再在班级内批量创建学生账号。
3. 教师在班级内上传参考 `sb3`，并填写项目标题、教学目标、说明。
4. `Go API` 保存原始 `sb3`，创建项目记录，并异步分析 `project.json`。
5. 教师等待分析状态变成 `ready`，再把项目分配给一个或多个同班学生。
...
教师 Web 当前已支持“下载真实 Excel `xlsx` 模板 + 粘贴表格数据”的批量导入交互；模板内会标出从哪一行开始填写、复制哪些列，以及如何粘贴导入。底层仍复用这个 JSON 批量接口，不额外新增文件上传型 API。
